 ...
 * */
    ...
    today: function () {
        var now = new Date();
        return now.getFullYear() + "-" + util.formatDateNumber(now.getMonth() + 1) + "-" + util.formatDateNumber(now.getDate());
    },
    ...
    saveJianshu: function (url, data) {
        io.read(url, function (json) {
            var len = json.length;
            if (json[len - 1].time == util.today()) {
                json[len - 1] = {
                    time: util.today(),
                    fans: data.fans,
